"""
Isolated PDF builder for 23_MSRX_SUPER_TRANSFER_VISUAL_DIAGRAMS.md
Does NOT modify application repos or the source Markdown architecture.
Improves readability via correct orientation + custom tall pages when needed.
"""
...
SRC = ROOT / "23_MSRX_SUPER_TRANSFER_VISUAL_DIAGRAMS.md"
BUILD = ROOT / "_pdf_render"
DIAG_DIR = BUILD / "diagrams"
OUT_PDF = ROOT / "23_MSRX_SUPER_TRANSFER_VISUAL_DIAGRAMS.pdf"

# Wide diagrams → landscape A4
LANDSCAPE_IDS = {2, 4, 10, 11, 12, 14, 15}
# Very tall flowcharts → custom tall portrait pages (keep text readable)
TALL_IDS = {1, 3, 5, 6, 7, 8, 9, 13}
...
def add_header_footer(canvas, doc, pagesize):
    ...
    canvas.drawString(15 * mm, page_h - 10 * mm, "BLUE WATER / MSRX  —  MSRX → Super Transfer Visual Architecture")
...
def build_cover(styles):
    ...
    story.append(Paragraph("BLUE WATER / MSRX", styles["CoverTitle"]))
    story.append(Spacer(1, 0.25 * inch))
    story.append(Paragraph("MSRX → SUPER TRANSFER", styles["CoverTitle"]))
    story.append(Spacer(1, 0.35 * inch))
    story.append(
        Paragraph(
            "Complete End-to-End Architecture<br/>&amp; Worker Processing Flow",
            styles["CoverSub"],
        )
    )
    story.append(Spacer(1, 0.5 * inch))
    story.append(Paragraph("Developer KT / Onboarding", styles["CoverSub"]))
    story.append(Spacer(1, 0.35 * inch))
    story.append(
        Paragraph(
            "Source: <font face='Courier'>23_MSRX_SUPER_TRANSFER_VISUAL_DIAGRAMS.md</font><br/>"
...
def main():
    ...
    if len(sections) != 15:
        raise RuntimeError(f"Expected 15 diagram sections, found {len(sections)}")
    ...
    doc = BaseDocTemplate(
        str(OUT_PDF),
        title="MSRX → Super Transfer Visual Diagrams",
